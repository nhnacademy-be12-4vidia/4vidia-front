...
public class OrderApiClient {
    ...
    public String saveTempOrderCheckout(List<OrderCheckoutRequest> orderCheckoutRequests) {
        ...
        String orderKey = backendApiClient.post(ORDER_SERVICE + "/orders/checkout-temp", orderCheckoutListRequest, new ParameterizedTypeReference<>() {});
        return orderKey;
    }

    // 주문 화면에 필요한 정보 가져오기
    public OrderCheckoutResponse getOrderCheckout(String orderKey) {
        OrderCheckoutResponse orderCheckoutResponse = backendApiClient.get(ORDER_SERVICE + "/orders?key=" + orderKey, new ParameterizedTypeReference<>() {});
        return orderCheckoutResponse;
    }

    // 주문 생성
    public OrderCreateResponse saveOrder(@Valid OrderCreateRequest orderCreateRequest) {
        return backendApiClient.post(ORDER_SERVICE + "/orders", orderCreateRequest, new ParameterizedTypeReference<>() {});
    }

    //Order 한개 내역 (연결된 오더아이템도)가져오기
    public OrderResponse getOrderById(long orderId) {
        OrderResponse orderResponse = backendApiClient.get(ORDER_SERVICE + "/orders/" + orderId, new ParameterizedTypeReference<>() {});
        return orderResponse;
    }

    // 결제에 필요한 Order 한개에 대한 실제 결제금액 가져오기
    public OrderAmountResponse getOrderPayPriceById(long orderId) {
        OrderAmountResponse orderAmountResponse = backendApiClient.get(ORDER_SERVICE + "/orders/" + orderId + "/amount",  new ParameterizedTypeReference<>() {});
    ...
    public OrderCountResponse getOrderCounts() {
        return backendApiClient.get(ORDER_SERVICE + "/users/me/orders/counts", new ParameterizedTypeReference<>() {});
    }

    public void cancelOrder(long orderId) {
        backendApiClient.putNoBody(ORDER_SERVICE + "/orders/" + orderId + "/cancel", new ParameterizedTypeReference<ApiResponse<Void>>() {});
    }

    public OrderResponse getGuestOrder(OrderTrackingRequest orderTrackingRequest) {
        return backendApiClient.post(ORDER_SERVICE + "/orders/guest", orderTrackingRequest, new ParameterizedTypeReference<>() {});
    }
}
